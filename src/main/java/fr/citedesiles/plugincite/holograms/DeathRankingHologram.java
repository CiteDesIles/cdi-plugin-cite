...
public class DeathRankingHologram {
    public static void init() {
        ...
        textHologramData.setText(List.of("§6§k§lClassement des morts", "§7Loading..."));
    ...
    public static void refresh() {
        ...
        HologramManager hologramManager = FancyHologramsPlugin.get().getHologramManager();
        Hologram hologram = hologramManager.getHologram("cdi.deathranking").orElse(null);
        if (hologram == null) {
            return;
        }
        ...
        List<String> lines = new ArrayList<>();
        lines.add("§6§l§kClassement des morts");
        int position = 1;
        ...
        for(CDITeam team : PluginCite.instance().teamManager().getTeams()) {
            ...
            for (CDITeam _team : PluginCite.instance().teamManager().getTeams()) {
                CDIObjectif angel = PluginCite.instance().objectifManager().getObjectif(_team.getName(), "death");
