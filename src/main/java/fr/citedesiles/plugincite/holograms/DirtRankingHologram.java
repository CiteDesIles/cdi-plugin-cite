...
public class DirtRankingHologram {
    public static void init() {
        HologramManager hologramManager = FancyHologramsPlugin.get().getHologramManager();
        TextHologramData textHologramData = new TextHologramData("cdi.dirtranking", new Location(Bukkit.getWorld("world"), 77.5, 111, -369.5));
        ...
        textHologramData.setText(List.of("§6§k§lXXXXXXXXXX XXX XXXXXXXXX XX XXXXXX", "§7Loading..."));
    ...
    public static void refresh() {
        ...
        HologramManager hologramManager = FancyHologramsPlugin.get().getHologramManager();
        Hologram hologram = hologramManager.getHologram("cdi.dirtranking").orElse(null);
        if (hologram == null) {
            return;
        }
        ...
        for(CDITeam team : PluginCite.instance().teamManager().getTeams()) {
            ...
            for (CDITeam _team : PluginCite.instance().teamManager().getTeams()) {
                CDIObjectif angel = PluginCite.instance().objectifManager().getObjectif(_team.getName(), "dirt");
