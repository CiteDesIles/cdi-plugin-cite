...
public class EnderpearlRankingHologram {
    public static void init() {
        ...
        textHologramData.setText(List.of("§6§k§lXXXXXXXXXX XXX XXXXXXX XX XXXXXX XX X'XXXXX", "§7Loading..."));
    ...
    public static void refresh() {
        ...
        HologramManager hologramManager = FancyHologramsPlugin.get().getHologramManager();
        Hologram hologram = hologramManager.getHologram("cdi.enderranking").orElse(null);
        if (hologram == null) {
            return;
        }
        List<String> lines = new ArrayList<>();
        lines.add("§6§l§kXXXXXXXXXX XXX XXXXXXX XX XXXXXX XX X'XXXXX");
        int position = 1;
        ...
        for(CDITeam team : PluginCite.instance().teamManager().getTeams()) {
            ...
            for (CDITeam _team : PluginCite.instance().teamManager().getTeams()) {
                CDIObjectif angel = PluginCite.instance().objectifManager().getObjectif(_team.getName(), "ender_pearl");
