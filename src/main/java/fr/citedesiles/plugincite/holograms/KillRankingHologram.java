...
public class KillRankingHologram {
    public static void init() {
        HologramManager hologramManager = FancyHologramsPlugin.get().getHologramManager();
        TextHologramData textHologramData = new TextHologramData("cdi.killranking", new Location(Bukkit.getWorld("world"), 103.5, 111, -369.5));
    ...
    public static void refresh() {
        ...
        HologramManager hologramManager = FancyHologramsPlugin.get().getHologramManager();
        Hologram hologram = hologramManager.getHologram("cdi.killranking").orElse(null);
        if (hologram == null) {
            return;
        }
        ...
        for(CDITeam team : PluginCite.instance().teamManager().getTeams()) {
            ...
            for (CDITeam _team : PluginCite.instance().teamManager().getTeams()) {
                CDIObjectif angel = PluginCite.instance().objectifManager().getObjectif(_team.getName(), "kill");
