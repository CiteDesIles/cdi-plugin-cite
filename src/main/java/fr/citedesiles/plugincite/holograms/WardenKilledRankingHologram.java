...
public class WardenKilledRankingHologram {
    public static void init() {
        ...
        textHologramData.setText(List.of("§6§k§lXXXXXXXXXX XXX XXXXXX XX XXXXX", "§7Loading..."));
    ...
    public static void refresh() {
        ...
        HologramManager hologramManager = FancyHologramsPlugin.get().getHologramManager();
        Hologram hologram = hologramManager.getHologram("cdi.wardenkillranking").orElse(null);
        if (hologram == null) {
            return;
        }
        ...
        List<String> lines = new ArrayList<>();
        lines.add("§6§l§kXXXXXXXXXX XXX XXXXXX XX XXXXX");
        int position = 1;
        ...
        for(CDITeam team : PluginCite.instance().teamManager().getTeams()) {
            ...
            for (CDITeam _team : PluginCite.instance().teamManager().getTeams()) {
                CDIObjectif angel = PluginCite.instance().objectifManager().getObjectif(_team.getName(), "wardenkilled");
