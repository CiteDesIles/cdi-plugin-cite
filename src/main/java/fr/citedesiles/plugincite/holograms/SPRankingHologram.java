...
public class SPRankingHologram {
    public static void init() {
        HologramManager hologramManager = FancyHologramsPlugin.get().getHologramManager();
        TextHologramData textHologramData = new TextHologramData("cdi.spranking", new Location(Bukkit.getWorld("world"), 68.5, 124, -339.5));
        ...
        textHologramData.setText(List.of("§6§k§lXXXXXXXXXX XXX XXXXXXXXXXXXX", "§7Loading..."));
    ...
    public static void refresh() {
        HologramManager hologramManager = FancyHologramsPlugin.get().getHologramManager();
        Hologram hologram = hologramManager.getHologram("cdi.spranking").orElse(null);
        if (hologram == null) {
            return;
        }
        List<String> lines = new ArrayList<>();
        lines.add("§6§l§kXXXXXXXXXX XXX XXXXXXXXXXXXX");
        ...
        for (CDITeam team : PluginCite.instance().teamManager().getTeams()) {
            ...
            int bestSP = Integer.MIN_VALUE;
            for(CDITeam _team : PluginCite.instance().teamManager().getTeams()) {
                if (_team.getSupportPoints() > bestSP && !teamListalreadysorted.contains(_team)) {
                    bestSP = _team.getSupportPoints();
                    curerentBest = _team;
                }
            }
            if (curerentBest != null) {
                ...
                lines.add("§7" + position + "e. " + curerentBest.getDisplayName() + " §8- §7" + curerentBest.getSupportPoints());
                position++;
            }
        }
